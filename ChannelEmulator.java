...
public class ChannelEmulator {
	...
	private static final long MAX_INACTIVE_TIME = 1800000; // milliseconds
	// after which user state is garbage collected
	private static final int MAX_Q_SIZE = 16; // max number of outstanding
	// segments per client
	private static final int MAX_TQ_SIZE = 10000; // max number of total
	// outstanding segments
	private static final int MAX_MAP_SIZE = 1000000; // max number of client
	// state entries
	...
	private static double DELAY_DEV_RATIO = 1.0; // ratio of deviation to
	// average delay
	private static double CORRUPTION = 0.01; // probability of corruption of
	// exactly one byte in each 100 byte block
	private static boolean PRIVILEGED_MODE = false; // If true, only
	// privileged_user can set CHNL parameters
	...
	private static enum Commands {
		CONN, NAME, OK, QUIT, LIST, CHNL, PARAMS, LOSS, DELAY, CORRUPTION,
		DELAY_DEV_RATIO
	}
	...
	/*
	 * The two hashmaps below need to be ConcurrentHashMaps as they are
	 * modified by
	 * the main thread as well as the garbage collector thread.
	 */
	private ConcurrentHashMap<InetSocketAddress, ChannelInfo> sockToCinfo =
			null; // [IP,port] -> channel info
	private ConcurrentHashMap<String, InetSocketAddress> nameToSock = null; //
	// name -> [IP,port]
	private GarbageCollector GC = null; // Garbage collects state left by
	// ungraceful client exits
	private Timer timer = null; // Used by GarbageCollector and Delayer to
	// schedule future events
	private Random random = null; // Used for implementing channel
	// characteristics
	...
	private class ChannelInfo {
		...
		public double delay_dev_ratio = DELAY_DEV_RATIO; // ratio of deviation
		// to average delay
		...
		public long lastActive; // time in milliseconds, used for garbage
		// collection upon inactivity
	...
	private class GarbageCollector extends TimerTask {
		public void run() {
			...
			for (InetSocketAddress isaddr : sockToCinfo.keySet()) {
				...
				if (cinfo != null && cinfo.lastActive < System
						.currentTimeMillis() - MAX_INACTIVE_TIME)
					cleanup(isaddr);
			}
		}
	}
	...
	private class Delayer extends TimerTask {
		...
		public void run() {
			try {
				...
				log.warning("IOException while sending delayed datagram: " +
						e);
			}
		}
	}
	...
	/*
	 * Synchronization here as well as in the cleanup method ensures the
	 * invariant
	...
	private synchronized void setName(String name, InetSocketAddress isaddr) {
		...
		cinfo.name = name;
		cinfo.lastActive = System.currentTimeMillis();
	...
	// Randomly corrupts a byte in each 100 byte block with corruption
	// probability
	private byte[] corrupt(byte[] msg, ChannelInfo cinfo) {
		...
		for (int i = 0; i < msg.length; i += 100) {
			if (random.nextDouble() < corruption) {
				int j = (int) (random.nextDouble() * (Math.min(i + 100, msg
						.length - 1)));
	...
	private void delay(DatagramPacket dgram, ChannelInfo srcCinfo) {
		...
		double src_delay_dev_ratio = (srcCinfo != null ? srcCinfo.delay :
				DELAY);
		double dst_delay = dstCinfo != null ? dstCinfo.delay : DELAY;
		double dst_delay_dev_ratio = (dstCinfo != null ? dstCinfo
				.delay_dev_ratio : DELAY_DEV_RATIO);
		timer.schedule(task, Math.max(0, (long) ((dst_delay * (1 +
				dst_delay_dev_ratio * (2 * random.nextDouble() - 1))) * 1000))
				+ Math.max(0, (long) ((src_delay * (1 + src_delay_dev_ratio *
				(2 * random.nextDouble() - 1))) * 1000)));
	}
	 ...
	 */
	private void mangle(DatagramPacket dgram, InetSocketAddress srcAddr)
			throws IOException {
		if (dgram == null || srcAddr == null)
			return; // dgram can be null if getRelayMessage returns null
		...
		if (msg != null && srcCinfo.getQSize() < MAX_Q_SIZE && getTQSize() <
				MAX_TQ_SIZE) {
			srcCinfo.incrQSize();
			this.incrTQSize();
			dgram.setData(msg);
	...
	private String processAsControlMessage(DatagramPacket dgram) {
		...
		if (isRelaying(dgram)) {
			if (cmd.equals(EOF) && parts.length == 1) {
				if (cinfo != null) cinfo.peer = null;
				response = Commands.OK.toString() + " Not relaying";
			}
		}
		// else not relaying, try processing as control message
		// first check for QUIT and happily clear away any state
		else if (cmd.equals(Commands.QUIT.toString()) && parts.length == 1) {
			response = Commands.OK.toString() + " Bye";
			cleanup(isaddr);
		}
		else if (cmd.equals(Commands.NAME.toString()) && parts.length == 2 &&
				(room = haveRoom(cinfo))) {
			setName(parts[1], isaddr);
			response = Commands.OK + " Hello " + parts[1];
		}
		else if (cmd.equals(Commands.CONN.toString()) && parts.length > 1 &&
				(room = haveRoom(cinfo))) {
			if (cinfo == null) cinfo = new ChannelInfo();
			...
			sockToCinfo.put(isaddr, cinfo);
			response = Commands.OK.toString() + " Relaying to " + cinfo.peer +
					((peerSock == null || sockToCinfo.get(peerSock) == null) ?
							" who is probably offline" : (!cinfo.peer.equals
							(peerSock.toString()) ? " at " + peerSock : ""));
		}
		/*
		 * The privileged_mode allows only the privileged_user to set channel
		 * parameters and sets it for all users.
		 */
		else if (cmd.equals(Commands.CHNL.toString()) && parts.length >= 3 &&
				(room = haveRoom(cinfo))) {
			if (cinfo == null) cinfo = new ChannelInfo();
			boolean parsed = true;
			for (int i = 1; i < parts.length - 1 && parsed; i += 2) {
				try {
					/*
					 * No checks for valid values. Probabilities below 0 or
					 * above 1 will be
					 * respectively treated as 0 and 1, and negative delays
					 * will be treated as
			...
			response = Commands.OK.toString() + " " + Commands.CHNL.toString()
				...
				(room = haveRoom(cinfo))) {
			response = Commands.OK.toString() + " " + Commands.LIST.toString()
					+ " = ";
	...
	private boolean allowCHNL(DatagramPacket dgram) {
		...
		if (cinfo != null && cinfo.name.equals(privileged_user)) return true;
		return false;
	}
	...
	private InetSocketAddress getPeerSock(String name) {
		...
		if (name != null && (isaddr = nameToSock.get(name)) == null) {
			...
			if (parts.length > 1) {
				try {
					InetAddress IP = InetAddress.getByName(parts[parts.length
							- 2]);
					...
					log.finer("Incorrectly formatted peer name: " + name);
				}
			}
		}
		return isaddr;
	...
	private DatagramPacket getRelayMessage(DatagramPacket dgram) {
		InetSocketAddress sndsock = (InetSocketAddress) (dgram
				.getSocketAddress());
		...
		if (cinfo != null && cinfo.peer != null) {
			...
			if (rcvsock != null) {
				...
				log.fine("Nullifying datagram because peerSock = " + rcvsock);
				dgram = null;
			}
		}
		return dgram;
		...
		for (int i = 0; i + 1 < args.length; i += 2) {
			...
			try {
				if (args[i].equals("-PORT") || args[i].equals("-P")) {
					...
					else if (args[i].equals("-DELAY") || args[i].equals
							("-D")) {
						DELAY = val;
					}
					else if (args[i].equals("-DELAY_DEV_RATIO") || args[i]
							.equals("-R")) {
						DELAY_DEV_RATIO = val;
					}
					else if (args[i].equals("-CORRUPTION") || args[i].equals
							("-C")) {
						LOSS = val;
					}
				}
			} catch (NumberFormatException nfe) {
				log.warning("Bad " + args[i] + " input, ignoring or using " +
						"default value");
				continue;
			}
		}
	 ...
	 * checks for relaying state, and otherwise simply echoes back to the
	 * sender.
	 */
	public void run() {
		int num_excepts = 0;
		...
		while (true) {
			try {
				...
				/* Only correctly formatted control messages elicit a response
				 from
				 * the server beginning with "OK". All other messages are
				 * either
				 * relayed or echoed back. In particular, there is no special
				 * "error"
				...
				// else check for relaying
					...
					log.fine("Relay message from " + sender + " to " +
							getPeerString(dgram) + " : " + truncate(dgram));
					InetSocketAddress isaddr = (InetSocketAddress) dgram
							.getSocketAddress();
				...
				// else simply echo back to sender
					...
					log.fine("Echo message from/to " + sender + ": " +
							truncate(dgram));
				}
			} catch (Exception e) {
				...
				try {
					...
					log.severe("Unable to recover from Exception, giving up: "
							+ efatal);
				}
			}
		}
	...
	private InetAddress getLocalAddress() {
		return this.udpsock.getLocalAddress();
	}
	...
	public static void main(String[] args) throws SocketException,
		...
		log.info("Starting chat server at " + chem.getLocalAddress() + ":" +
				serverPort + chem.printParams(null) + "\n");
		chem.run();
	}
}
